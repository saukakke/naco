...
{
    public function show(Request $request, Cadet $cadet, AuthorizationService $authorization)
    {
    ...
    public function enroll(Request $request, Cadet $cadet, Course $course, InstructorQualificationService $service, AuthorizationService $authorization)
    {
        abort_unless($authorization->canAccessCadet($request->user(), $cadet), 403);
    ...
    public function issueWarrant(Request $request, Cadet $cadet, Course $course, InstructorQualificationService $service)
    {
        ...
        return response()->json($service->issueWarrant($cadet,$course,(int)($data['validity_months']??24)),201);
    }
}
