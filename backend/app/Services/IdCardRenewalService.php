...
class IdCardRenewalService
{
    public function eligible(Cadet $cadet): bool
    {
        ...
        $expiry = Carbon::parse($cadet->id_card_expires_at);
        $now = now();
    ...
    {
        if (!$this->eligible($cadet)) {
            throw ValidationException::withMessages(['renewal' => 'ID card renewal can only be requested within two months of the current card expiry date.']);
        }
        if (IdCardRenewalApplication::where('cadet_id',$cadet->id)->whereIn('status',['pending','payment_pending','paid','approved'])->exists()) {
            throw ValidationException::withMessages(['renewal' => 'The cadet already has an active ID card renewal application.']);
        }
        return IdCardRenewalApplication::create([
            'cadet_id'=>$cadet->id,
            'reference'=>'NACO-IDR-'.now()->format('YmdHis').'-'.$cadet->id,
            'current_card_expires_at'=>$cadet->id_card_expires_at,
    ...
    public function issue(IdCardRenewalApplication $application): IdCardRenewalApplication
    {
        ...
        return DB::transaction(function () use ($application): IdCardRenewalApplication {
            $application->cadet()->update(['id_card_expires_at'=>now()->addYears(2)]);
