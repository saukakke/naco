...
class WardTransferService {
 public function apply(Cadet $cadet,int $toWardId,?string $reason=null):WardTransfer {
  $cadet->load('ward.lga.state'); $to=\App\Models\Ward::with('lga.state')->findOrFail($toWardId);
  ...
  if(WardTransfer::where('cadet_id',$cadet->id)->whereNotIn('status',['completed','rejected'])->exists()) throw ValidationException::withMessages(['transfer'=>'An active ward transfer already exists.']);
  return WardTransfer::create(['cadet_id'=>$cadet->id,'from_ward_id'=>$cadet->ward_id,'to_ward_id'=>$toWardId,'reference'=>'NACO-WT-'.now()->format('YmdHis').'-'.$cadet->id,'status'=>'pending_source_hcs','reason'=>$reason]);
 }
 ...
 public function sourceLga(WardTransfer $t,int $userId):WardTransfer { $this->state($t,'pending_source_lga'); $this->update($t,['status'=>$this->sourceStateRequired($t)?'pending_source_state':'pending_destination_hcs','source_lga_acknowledged_at'=>now(),'source_lga_acknowledged_by'=>$userId]); return $t->fresh(); }
 ...
 public function destinationHcs(WardTransfer $t,int $userId):WardTransfer { $this->state($t,'pending_destination_hcs'); $next=$this->destinationLgaRequired($t)?'pending_destination_lga':($this->destinationStateRequired($t)?'pending_destination_state':'pending_national'); $this->update($t,['status'=>$next,'destination_hcs_accepted_at'=>now(),'destination_hcs_id'=>$userId]); return $t->fresh(); }
 ...
 private function sourceStateRequired(WardTransfer $t):bool{$t->loadMissing('fromWard.lga.state','toWard.lga.state');return $t->fromWard->lga->state_id!==$t->toWard->lga->state_id;}
