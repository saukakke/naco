...
{
    public function apply(Cadet $cadet,int $toWardId,?string $reason=null):WardTransfer
    {
        $cadet->load('ward.lga.state');
        $destination=Ward::with('lga.state')->findOrFail($toWardId);
        if (!$cadet->ward_id) throw ValidationException::withMessages(['ward'=>'The cadet has no current ward assigned.']);
        if ((int)$cadet->ward_id === $toWardId) throw ValidationException::withMessages(['to_ward_id'=>'The destination ward must be different.']);
        if (WardTransfer::where('cadet_id',$cadet->service_number)->whereNotIn('status',['completed','rejected'])->exists()) throw ValidationException::withMessages(['transfer'=>'An active ward transfer already exists.']);
        return WardTransfer::create(['cadet_id'=>$cadet->service_number,'from_ward_id'=>$cadet->ward_id,'to_ward_id'=>$destination->id,'reference'=>'NACO-WT-'.now()->format('YmdHis').'-'.bin2hex(random_bytes(3)),'status'=>'pending_source_hcs','reason'=>$reason]);
    }
    ...
    public function nationalApprove(WardTransfer $t,int $userId):WardTransfer{$this->state($t,'pending_national');return DB::transaction(function()use($t,$userId):WardTransfer{$t->refresh();if($t->status!=='pending_national')throw ValidationException::withMessages(['transfer'=>'This transfer has already been processed.']);$t->cadet()->update(['ward_id'=>$t->to_ward_id]);$this->update($t,['status'=>'completed','national_approved_at'=>now(),'national_approved_by'=>$userId,'completed_at'=>now()]);return $t->fresh();});}
    ...
    private function destinationLgaRequired(WardTransfer $t):bool{$t->loadMissing('fromWard.lga','toWard.lga');return (int)$t->fromWard->lga_id !== (int)$t->toWard->lga_id;}
    private function destinationStateRequired(WardTransfer $t):bool{$t->loadMissing('fromWard.lga.state','toWard.lga.state');return (int)$t->fromWard->lga->state_id !== (int)$t->toWard->lga->state_id;}
}
