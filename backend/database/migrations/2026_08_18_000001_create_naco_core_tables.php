...
return new class extends Migration {
    public function up(): void
    {
        ...
        Schema::create('cadet_course', function (Blueprint $table): void {
            $table->id(); $table->foreignId('cadet_id')->constrained()->cascadeOnDelete(); $table->foreignId('course_id')->constrained()->cascadeOnDelete(); $table->string('status')->default('enrolled'); $table->date('completed_at')->nullable(); $table->string('result')->nullable(); $table->timestamps(); $table->unique(['cadet_id','course_id']);
        });
        ...
        Schema::create('post_assignments', function (Blueprint $table): void {
            $table->id(); $table->foreignId('cadet_id')->constrained()->cascadeOnDelete(); $table->foreignId('post_id')->constrained()->cascadeOnDelete(); $table->date('start_date'); $table->date('end_date')->nullable(); $table->string('status')->default('active'); $table->string('reference')->nullable()->unique(); $table->timestamps();
        });
        Schema::create('warrants', function (Blueprint $table): void {
            $table->id(); $table->foreignId('cadet_id')->constrained()->cascadeOnDelete(); $table->foreignId('course_id')->constrained()->restrictOnDelete(); $table->string('warrant_number')->unique(); $table->string('type'); $table->date('issued_at'); $table->date('expires_at')->nullable(); $table->string('status')->default('valid'); $table->string('document_path')->nullable(); $table->timestamps();
        });
        Schema::create('promotions', function (Blueprint $table): void {
            $table->id(); $table->foreignId('cadet_id')->constrained()->cascadeOnDelete(); $table->foreignId('from_rank_id')->constrained('ranks')->restrictOnDelete(); $table->foreignId('to_rank_id')->constrained('ranks')->restrictOnDelete(); $table->date('promoted_at'); $table->text('reason')->nullable(); $table->string('reference')->unique(); $table->string('document_path')->nullable(); $table->string('status')->default('approved'); $table->timestamps();
        });
        Schema::create('demotions', function (Blueprint $table): void {
            $table->id(); $table->foreignId('cadet_id')->constrained()->cascadeOnDelete(); $table->foreignId('from_rank_id')->constrained('ranks')->restrictOnDelete(); $table->foreignId('to_rank_id')->constrained('ranks')->restrictOnDelete(); $table->date('demoted_at'); $table->text('reason')->nullable(); $table->string('reference')->unique(); $table->string('document_path')->nullable(); $table->string('status')->default('approved'); $table->timestamps();
        });
    }

    public function down(): void
    {
        foreach (['demotions','promotions','warrants','post_assignments','posts','cadet_course','courses','cadets','ranks','rank_categories','units'] as $table) Schema::dropIfExists($table);
    }
};
